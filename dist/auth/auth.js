...
class Auth {
    static serializeUser() {
        ...
        passport.deserializeUser(function (id, done) {
            User_1.User.find({ where: { id } }).then(function (user) {
                done(null, user);
            });
        });
    }
                    ...
                    if (jwtSecret) {
                        jsonwebtoken_1.verify(accessToken.token, jwtSecret, (err, decodedToken) => {
                            if (decodedToken && accessToken.userId === decodedToken.id) {
                                User_1.User.find({ where: { id: accessToken.userId } }).then(user => {
                                    return done(null, user);
                                }).catch(error => {
                                    return done(new AuthError_1.AuthError(error.message), false);
                                });
                            }
                            else {
                                done(new AuthError_1.AuthError(err.message), false);
                            }
                        });
                    }
                }
                else {
                    return done(new AuthError_1.AuthError("Unauthorized"), false);
                }
    ...
    static useBasicStrategy() {
        passport.use(new passport_http_1.BasicStrategy(function (clientId, clientSecret, done) {
